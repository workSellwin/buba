...
$rsEnum = $obEnum->GetList([], ["USER_FIELD_ID" => 38]);
while($arEnum = $rsEnum->Fetch())
{
	$arSiteIds[$arEnum['ID']] = $arEnum['VALUE'];
}
...
{
	$arSites[ $arSite['LID'] ] = '//' . $arSite['SERVER_NAME'];
}

$currentSite = 1;
foreach($arSiteIds as $key => $id)
{
	if($id == SITE_ID)
	{
		$currentSite = $key;
...
foreach($arResult['SECTIONS'] as $key => $arSection)
{
	if(!in_array($currentSite, $arSection['UF_SITE']))
	{
		$elemSite = current($arSection['UF_SITE']);
		$elemSid = $arSiteIds[$elemSite];

		$arResult['SECTIONS'][$key]['SECTION_PAGE_URL'] = $arSites[$elemSid] . $arSection['SECTION_PAGE_URL'];
	}
}
?>