...
function changeCalendar(flag) {
    ...
    if($('[name="DELIVERY_ID"]:checked').val()==3){
        var hour = new Date().getUTCHours() + utc;
    ...
    /** todo и пожалуйста, исправьте, чтобы сегодня после 15.00 нельзя было на сегодня оформить заказ **/
    var hour = new Date().getUTCHours() + utc,
        currentDateStr = date.getUTCDate()+'.'+(date.getUTCMonth()+1)+'.'+date.getUTCFullYear();

    if(hour > 14 && currentDateStr == '29.12.2018'){
        startDateIndent = 1;
    }
    /** endtodo**/
