...
if (!empty($arResult['ITEMS']))
{
	...
	<? if(empty($arParams['HIDE_BLOCK_TITLE']) || $arParams['HIDE_BLOCK_TITLE'] !== 'Y'){ ?><div class="h1"><? echo ($arParams['BLOCK_TITLE']? htmlspecialcharsbx($arParams['BLOCK_TITLE']) : GetMessage('SGB_TPL_BLOCK_TITLE_DEFAULT')) ?></div><? } ?>
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		<div class="prod-item">
			...
			{
				?><a id="<? echo $arItemIDs['SECOND_PICT']; ?>" href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" class="hidden hide"<? if ($arParams['SHOW_IMAGE'] == "Y")
			{
