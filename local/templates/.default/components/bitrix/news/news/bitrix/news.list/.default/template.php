		...
		<?foreach($arResult["ITEMS"] as $arItem):?>
			...
			<div class="news-list__item" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
				<?if($arItem["PREVIEW_PICTURE"]):?>
					<a class="news-list__img" href="<?=$arItem["DETAIL_PAGE_URL"]?>" title="<?=$arItem["NAME"]?>">
				...
				<div class="news-list__info">
					...
					<?endif?>
					<a class="btn btn_border" href="<?=$arItem["DETAIL_PAGE_URL"]?>" title="<?=$arItem["NAME"]?>"><?=Loc::getMessage("T_SERVICES_MORE")?></a>
				</div>
			</div>
		<?endforeach;?>
	</div>
