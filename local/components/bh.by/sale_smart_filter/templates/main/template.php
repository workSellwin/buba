		...
		<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter">
			...
			foreach($arResult["ITEMS"] as $key=>$arItem)
			{
				...
				if (
					...
					continue;
				if($arItem["CODE"] == "COLOR_REF" || empty($arItem["NAME"])) continue;
				?>
						...
						switch ($arItem["DISPLAY_TYPE"])
						{
								...
								?>
									<div class="checkbox">
										<input
											...
											class="radio-field"
										/>
										<label class="radio__label bx-filter-param-label" for="<? echo "all_".$arCur["CONTROL_ID"] ?>">
									...
									<?foreach($arItem["VALUES"] as $val => $ar):?>
										<div class="checkbox">
											<input
												...
												class="radio-field"
											/>
											<label data-role="label_<?=$ar["CONTROL_ID"]?>" class="radio__label bx-filter-param-label" for="<? echo $ar["CONTROL_ID"] ?>">
							...
							default://CHECKBOXES
									...
									if($i<=9):?>
										<div class="checkbox">
											<input
												...
												onclick="smartFilter.click(this)"
											/>
											<label data-role="label_<?=$ar["CONTROL_ID"]?>" class="checkbox__label bx-filter-param-label <? echo $ar["DISABLED"] ? 'disabled': '' ?>" for="<? echo $ar["CONTROL_ID"] ?>">
												<span class="bx-filter-input-checkbox">
								...
								<?if($i>9):?>
									...
									<a href="#" class="show-checkbox" data-text-hide="скрыть" data-text-show="показать все">показать все</a>
								<?endif;?>
						<?
						}
						?>
...
<? $this->SetViewTarget('prop_filter'); ?>
<? foreach ($arResult['PROP_FILTER'] as $key => $arItems): ?>
    <? if ($key == 'PRICE'): ?>
        <? //PR($arItems); ?>
        <? foreach ($arItems as $items): ?>
            ...
            <? foreach ($items as $item): ?>
                <? //PR($item); ?>
                <div class="schema-tags__item" data-price="Y" data-type="<?= $item['TYPE'] ?>"
                     data-encoded-id="<?= $item['ENCODED_ID'] ?>" data-price-value="<?= round($item['VALUE']) ?>"
                     data-prop-id="<?= $item['CONTROL_ID'] ?>">
                    <span class="schema-tags__text"><?= $item['NAME'] . $item['HTML_VALUE'] ?></span>
                </div>
            <? endforeach; ?>
        <? endforeach; ?>
    <? else: ?>
        <? foreach ($arItems as $items): ?>
            <? foreach ($items as $item): ?>
                <div class="schema-tags__item" data-prop-id="<?= $item['CONTROL_ID'] ?>">
                    <span class="schema-tags__text"><?= $item['VALUE'] ?></span>
                </div>
            <? endforeach; ?>
        <? endforeach; ?>
    <? endif; ?>
...
<? foreach ($arResult['DEF_PRICE'] as $k => $items): ?>
    <? foreach ($items as $key => $item): ?>
        <script>
            $(document).ready(function () {
                setTimeout(function () {
                    if ('<?=$key?>' == 'MIN') {
                        $('#left_slider_<?=$k?>').css('left', '0');
                        $('#colorUnavailableActive_<?=$k?>').css('left', '0');
                        $('#colorAvailableInactive_<?=$k?>').css('left', '0');
                        $('#colorAvailableActive_<?=$k?>').css('left', '0');
                    }
                    if ('<?=$key?>' == 'MAX') {
                        $('#right_slider_<?=$k?>').css('right', '0');
                        $('#colorUnavailableActive_<?=$k?>').css('right', '0%');
                        $('#colorAvailableInactive_<?=$k?>').css('right', '0%');
                        $('#colorAvailableActive_<?=$k?>').css('right', '0%');
                    }
                }, 1000);
            });
        </script>
